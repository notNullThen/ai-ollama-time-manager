...
- `AiInteraction` connects to local Ollama through the sibling `AIOrchestrator` project, referenced from `TimeCalculator.csproj`. `AiAppFacade` exposes time-entry mutations as LLM-callable tools.
...
Keep `ai-orchestrator-dotnet` beside this repository. `dotnet build` and `dotnet run` restore project dependencies automatically; use `dotnet restore TimeCalculator.slnx` only when an explicit restore step is useful. Ollama must be running for AI behavior; ordinary builds do not require it. The repository currently has no automated test project, so at minimum build after code changes and manually exercise affected Blazor/AI flows when relevant. A local CSharpier tool is declared in `dotnet-tools.json`; restore that tool separately with `dotnet tool restore` before formatting.
